    ...
    <div className="bg-card rounded-xl border border-border p-4">
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          {/* Desktop clear all — inline with search */}
          {hasActiveFilters && (
            <button
              onClick={clearAll}
        ...
        {/* Filter rows — always visible on md+, toggleable on mobile */}
        <div className={showFilters ? 'flex flex-col gap-3' : 'hidden md:flex md:flex-col md:gap-3'}>
          ...
          {/* Row 3: Score, Essential & Sort */}
          <div className="flex flex-col sm:flex-row gap-2">
            <select value={currentScoreFilter} onChange={e => updateParams('score_filter', e.target.value)} className={`flex-1 ${selectClass}`}>
            ...
            <select value={currentEssentialFilter} onChange={e => updateParams('essential_filter', e.target.value)} className={`flex-1 ${selectClass}`}>
              <option value="all">All Materials</option>
              <option value="essential">💎 Essential Only</option>
              <option value="non_essential">Regular Only</option>
            </select>

            <select value={currentSort} onChange={e => updateParams('sort', e.target.value)} className={`flex-1 ${selectClass}`}>
            ...
            {/* Mobile clear all — inside panel */}
            {hasActiveFilters && (
              <button
                onClick={clearAll}
                className="md:hidden px-4 py-2 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg font-medium transition-colors whitespace-nowrap border border-red-200"
              >
                Clear All
              </button>
            )}
